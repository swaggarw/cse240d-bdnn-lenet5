	...
	if (f.is_open()) {
		while( index < numSamples ) {
			...
			while ( pos2 < (int)line.length()+1 ) {
				if (pos2 == 0) {
					pos2 = line.find(" ", pos1);
					uint32_t temp = std::atoi((line.substr(pos1, pos2-pos1)).c_str());
					...
					pos2 = line.find(" ", pos1);
					column = atof((line.substr(pos0+1, pos1-pos0-1)).c_str());
					if (pos2 == -1) {
						pos2 = line.length()+1;
						x[index*numFeatures + column] = atof((line.substr(pos1, pos2-pos1)).c_str());
					}
					else
						x[index*numFeatures + column] = atof((line.substr(pos1, pos2-pos1)).c_str());
				}
